...
const avlPicksVenueId = "avl-picks";
const avlPicksLabel = "AVL Picks";
...
const venueConfigs = [
  {
  ...
  },
    {
      ...
      parser: parsePosterRhpVenueEvents,
    },
    {
      id: "grey-eagle",
    ...
    },
  {
      ...
      parser: parsePosterHarrahsVenueEvents,
    },
    {
    ...
    },
    {
      ...
      parser: parsePosterRevivalVenueEvents,
    },
];
...
async function getVenueEvents(venueId) {
  if (venueId === avlPicksVenueId) {
    return getAvlPicksEvents();
  }
  ...
  const events = venueConfig
    .parser(sourceText, venueConfig)
    .slice(0, 6)
    .map((event) => ({
      ...event,
...
async function getAvlPicksEvents() {
  const fetchedAt = new Date().toISOString();
  const settledResults = await Promise.allSettled(
    venueConfigs.map((venueConfig) => getVenueEvents(venueConfig.id)),
  );
  const eventGroups = [];
  ...
  return {
    venue: avlPicksVenueId,
    label: avlPicksLabel,
    fetchedAt,
    events: dedupeVenueEvents(interleaveEventGroups(eventGroups, 6)),
  };
}

function interleaveEventGroups(eventGroups, limit) {
  const queues = eventGroups.map((events) => [...events]);
  const results = [];
...
function parsePosterRhpVenueEvents(html, venueConfig) {
  return parseRhpVenueEvents(html).map((event) => ({
    ...event,
    url: toAbsoluteUrl(venueConfig.sourceUrl, event.url),
    metaText: normalizeText(event.metaText).replace(/\s*[Â·•]\s*/g, " - "),
    posterUrl: extractImageUrlFromUrlBlock(html, event.url, venueConfig.sourceUrl),
  }));
}

function parsePosterHarrahsVenueEvents(html, venueConfig) {
  return parseHarrahsVenueEvents(html).map((event) => ({
    ...event,
    url: toAbsoluteUrl(venueConfig.sourceUrl, event.url),
    posterUrl: extractImageUrlFromUrlBlock(html, event.url, venueConfig.sourceUrl),
  }));
...
function parsePosterRevivalVenueEvents(html, venueConfig) {
  return parseRevivalVenueEvents(html).map((event) => ({
    ...event,
    url: toAbsoluteUrl(venueConfig.sourceUrl, event.url),
    posterUrl: extractImageUrlFromUrlBlock(html, event.url, venueConfig.sourceUrl),
  }));
...
function toAbsoluteUrl(baseUrl, maybeRelativeUrl) {
  ...
  try {
    return new URL(decodeHtmlEntities(maybeRelativeUrl), baseUrl).toString();
  } catch {
    return decodeHtmlEntities(maybeRelativeUrl);
  }
}

function extractImageUrlFromUrlBlock(html, eventUrl, baseUrl) {
  const normalizedEventUrl = toAbsoluteUrl(baseUrl, eventUrl);

  if (normalizedEventUrl) {
    const escapedEventUrl = escapeForRegExp(eventUrl)
      || escapeForRegExp(normalizedEventUrl)
      || "";
    if (escapedEventUrl) {
      const nearbyPattern = new RegExp(
        `<a[^>]+href="${escapedEventUrl}"[\\s\\S]{0,4000}?<img[^>]+(?:src|data-src)="([^"]+)"`,
        "i",
      );
      const nearbyMatch = html.match(nearbyPattern)?.[1];
      if (nearbyMatch) {
        return toAbsoluteUrl(baseUrl, nearbyMatch);
      }
    }
  }

  const firstImage =
    getFirstMatch(html, /<img[^>]+src="([^"]+)"/i) ||
    getFirstMatch(html, /<img[^>]+data-src="([^"]+)"/i) ||
    getFirstMatch(html, /<img[^>]+srcset="([^"]+)"/i).split(",")[0]?.trim().split(/\s+/)[0] ||
    "";

  return toAbsoluteUrl(baseUrl, firstImage);
}
...
const server = createServer(async (request, response) => {
  ...
  if (requestUrl.pathname === "/api/venue-events") {
    ...
    if (venueId !== avlPicksVenueId && !venueConfig) {
    ...
    try {
      ...
      response.end(
        JSON.stringify({
          error: `Unable to load events for ${(venueConfig && venueConfig.label) || avlPicksLabel} right now.`,
        }),
      );
    }
    return;
...
server.listen(port, () => {
  console.log(`Quiet Playground server running on port ${port}`);
});
